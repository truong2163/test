...
namespace Nhom11_Quanlybangiay.Khachhang
{
    public partial class frmkhachhang : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng cung cấp đầy đủ thông tin khách hàng!!!","Lưu ý",MessageBoxButtons.OK,MessageBoxIcon.Warning);
            }    
            else
            {
                if(string.IsNullOrWhiteSpace(txtdiachi.Text)||string.IsNullOrWhiteSpace(txtsdt.Text)||string.IsNullOrWhiteSpace(txttenkh.Text))
                {
                ...
                {
                    if(data.xemKHTonTaiHayChua(txtsdt.Text)==true)
                    {
                        data.con.Close();
                        if (data.kiemtrahaithongtin(txtsdt.Text,txttenkh.Text,txtdiachi.Text)==true)
                        {
                            frmXuatHoaDon f = new frmXuatHoaDon(txtsdt.Text, txttenkh.Text, txtdiachi.Text);
                        ...
                        {
                            MessageBox.Show("Thông tin khách hàng chưa khớp với cơ sở dữ liệu!!!");
                            data.con.Close();
                        }    
                    }    
                    else
